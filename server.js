...
const responderWhatsAppConImagen = async (number, imageUrl, caption = "") => {
  try {
    await axios.post(`${process.env.EVOLUTION_API_URL}/message/sendImage/${process.env.EVOLUTION_INSTANCE}`,
      { number: number, url: imageUrl, caption: caption },
      { headers: { apikey: process.env.EVOLUTION_API_KEY } }
    );
  } catch (error) { console.error("❌ Error Imagen WA:", error.message); }
};
...
// ==========================================
// GESTIÓN DE PERFIL (SOLUCIÓN 404/401)
// ==========================================
...
app.delete("/api/auth/profile", authenticateUser, async (req, res) => {
  try {
    ...
    for (const s of sensors) { await Measurement.deleteMany({ sensorId: s.hardwareId }); }
...
// ==========================================
// RECEPCIÓN DE DATOS Y ESTADO HARDWARE
// ==========================================
app.post("/api/data", async (req, res) => {
  ...
  try {
    ...
    await new Measurement({ sensorId, temperatureC: Number(tempC), voltageV: Number(voltageV) }).save();
    ...
    if (tempC > sensor.alertThreshold && sensor.owner?.whatsapp) {
      ...
      if (!sensor.lastAlertSent || ahora - sensor.lastAlertSent > cooldownMs) {
        await sendWhatsAppAlert(sensor.owner.whatsapp, sensor.friendlyName, tempC);
...
app.post("/api/device/status", async (req, res) => {
  const { ip, oneWirePin, physicalSensors, configuredSensors, mapping } = req.body;
  lastDeviceStatus = { online: true, ip, oneWirePin, physicalSensors, configuredSensors, mapping, timestamp: new Date() };
  if (mapping) {
    for (const m of mapping) {
      await Sensor.findOneAndUpdate({ hardwareId: m.hardwareId }, { address: m.address });
    }
  }
...
app.post("/api/webhook/whatsapp", async (req, res) => {
  try {
    ...
    const text = (data.message.conversation || data.message.extendedTextMessage?.text || "").toLowerCase().trim();
    ...
    if (text === "estado") {
      ...
      for (const s of sensors) {
        const lastM = await Measurement.findOne({ sensorId: s.hardwareId }).sort({ timestamp: -1 });
        const temp = lastM ? `${lastM.temperatureC}°C` : "N/A";
        const icon = lastM && lastM.temperatureC > s.alertThreshold ? "🔴" : "🟢";
    ...
    if (text.startsWith("historial")) {
      ...
      const sensor = await Sensor.findOne({ owner: user._id, friendlyName: { $regex: new RegExp(busqueda, "i") } });
      if (sensor) {
        const docs = await Measurement.find({ sensorId: sensor.hardwareId }).sort({ timestamp: -1 }).limit(10);
        if (docs.length > 0) {
          const chart = new QuickChart();
          chart.setConfig({
            type: "line",
            data: {
              labels: docs.map((m) => new Date(m.timestamp).toLocaleTimeString()).reverse(),
              datasets: [{ label: "Temp °C", data: docs.map((m) => m.temperatureC).reverse(), borderColor: "#36A2EB", fill: true, backgroundColor: "rgba(54, 162, 235, 0.2)" }],
            },
          });
          await responderWhatsAppConImagen(from, chart.getUrl(), `📊 Historial de ${sensor.friendlyName}`);
        }
      }
    }
    res.sendStatus(200);
...
app.get("/api/latest", authenticateUser, async (req, res) => {
  try {
    const data = await Sensor.aggregate([
      { $match: { owner: new mongoose.Types.ObjectId(req.user.id), enabled: true } },
      { $lookup: { from: "measurements", localField: "hardwareId", foreignField: "sensorId", as: "m" } },
      { $unwind: { path: "$m", preserveNullAndEmptyArrays: true } },
      { $sort: { "m.timestamp": -1 } },
      { $group: { _id: "$hardwareId", friendlyName: { $first: "$friendlyName" }, alertThreshold: { $first: "$alertThreshold" }, temperatureC: { $first: "$m.temperatureC" }, voltageV: { $first: "$m.voltageV" }, timestamp: { $first: "$m.timestamp" } } }
    ]);
    res.json(data);
  } catch (err) { res.status(500).send("Error"); }
});
...
app.get("/api/history", authenticateUser, async (req, res) => {
  try {
    ...
    const docs = await Measurement.find({ sensorId }).sort({ timestamp: -1 }).limit(Number(limit));
    res.json(docs);
  } catch (err) { res.status(500).json([]); }
});
...
app.post("/api/sensors/config", authenticateUser, async (req, res) => {
  try {
    ...
    const sensor = await Sensor.findOneAndUpdate({ hardwareId: req.body.hardwareId }, sensorData, { upsert: true, new: true });
    res.json(sensor);
...
app.delete("/api/sensors/:hardwareId", authenticateUser, async (req, res) => {
  try {
    const sensor = await Sensor.findOneAndDelete({ hardwareId: req.params.hardwareId, owner: req.user.id });
    if (sensor) await Measurement.deleteMany({ sensorId: sensor.hardwareId });
    res.json({ message: "Eliminado" });
  } catch (err) { res.status(500).send("Error"); }
});

app.get("/api/sensors/ids", authenticateUser, (req, res) => res.json(ESP_HARDWARE_IDS));
app.get("/api/device/status", authenticateUser, (req, res) => res.json(lastDeviceStatus));

app.listen(PORT, () => console.log(`🚀 Servidor Final desplegado en puerto ${PORT}`));